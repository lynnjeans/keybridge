...
enum PathBoxResolver {
    ...
    static func resolve(_ text: String, fileManager: FileManager = .default) -> PathBoxTarget? {
        ...
        // A bundle — an app, an .rtfd, a Keynote document — is a directory
        // on disk but a file to whoever pasted its path: opening it would
        // launch or open it, so it is revealed like any other file.
        guard isDirectory.boolValue, !isPackage(url) else { return .file(url) }
        return .folder(url)
    }

    /// Trims whitespace/newlines and one pair of wrapping quotes, the shape
    /// Windows' own "Copy as path" and a shell's `pwd` sometimes leave, and
    /// keeps the first line only: KeyBridge's own Copy Path writes one path
    /// per line, so a multiple selection pasted here goes to the first.
